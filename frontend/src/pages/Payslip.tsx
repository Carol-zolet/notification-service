...
export function Payslip() {
  ...
  const fetchUnidades = async () => {
    try {
      const res = await fetch(`${config.apiBaseUrl}/api/admin/unidades`);
  ...
  const fetchColaboradores = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${config.apiBaseUrl}/api/admin/colaboradores?unidade=${encodeURIComponent(unidade)}`);
      const data = await res.json();
  ...
  const handleEnviar = async () => {
    ...
    try {
      ...
      const res = await fetch(`${config.apiBaseUrl}/api/payslips/distribuir`, {
        method: 'POST',
        body: formData,
      });
