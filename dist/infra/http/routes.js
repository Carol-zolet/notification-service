...
class ReprocessFailedNotificationsUseCase {
    prisma;
    emailService;
    constructor(prismaClient, emailSvc) {
    ...
    async execute(opts) {
        ...
        const results = await Promise.all(items.map(async (n) => {
            try {
...
// Unidades
exports.router.get("/unidades", async (_req, res) => {
...
// Colaboradores
exports.router.get("/colaboradores", async (_req, res) => {
...
exports.router.post("/notifications", async (req, res) => {
    try {
...
exports.router.get("/notifications", async (_req, res) => {
...
// Falhas
exports.router.get("/notifications/failed", async (req, res) => {
    try {
...
exports.router.post("/notifications/reprocess", async (req, res) => {
    try {
        const { limit, unidade, ids, batchSize, maxRetries, incrementalRetry, dryRun, } = req.body;
        let parsedIds;
...
exports.router.post("/payslips/process", upload.single("pdfFile"), async (req, res) => {
    try {
        const { unidade, subject = "Holerite", message = "Olá {{nome}}, segue holerite de {{unidade}}.", dryRun, confirm, batchSize, testRecipient, } = req.body;
        ...
        for (let i = 0; i < colaboradores.length; i += bs) {
            ...
            const results = await Promise.all(lote.map(async (c) => {
                try {
                    ...
                    await emailService.sendWithAttachments(c.email, // to
...
exports.router.get("/payslips/history", async (req, res) => {
    const { unidade, page = "1", limit = "20" } = req.query;
...
exports.router.get('/debug/total', async (req, res) => {
    try {
...
exports.router.get('/debug/unidades', async (req, res) => {
    try {
...
exports.router.get('/debug/colaboradores-por-unidade', async (req, res) => {
    try {
...
exports.router.get('/admin/colaboradores', async (req, res) => {
    try {
...
exports.router.get('/admin/unidades', async (req, res) => {
    try {
        const unidades = await prisma.colaborador.groupBy({
            by: ['unidade'],
            _count: { id: true },
...
exports.router.post('/admin/colaboradores', async (req, res) => {
    try {
...
exports.router.put('/admin/colaboradores/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { nome, email, unidade } = req.body;
...
exports.router.delete('/admin/colaboradores/:id', async (req, res) => {
    try {
        const { id } = req.params;
...
exports.router.post('/payslips/distribuir', upload.single('pdfFile'), async (req, res) => {
    try {
